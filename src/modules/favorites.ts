...
export const moveFavorite = async (item: Now, direction: 'up' | 'down'): Promise<Now[]> => {
  const currentState = await getFavorites()
  // get from and to
  const from: number = currentState.findIndex(currentItem => item.city+','+item.country === currentItem.city+','+currentItem.country)
  let to: number = from;
...
export const removeFromFavorites = async (item: Now): Promise<Now[]> => {
  const currentState = await getFavorites()
  const newState = currentState.filter((currentItem: Now) => item.city+','+item.country !== currentItem.city+','+currentItem.country)
  // set preferences
  setStored(newState)
